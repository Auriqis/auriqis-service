...
const Box: React.FC<BoxProps & { color: string; opacity: number }> = ({ position, rotation, color, opacity }) => {
...
const AnimatedBoxes = ({ boxColor, boxOpacity }: { boxColor: string; boxOpacity: number }) => {
...
export const Scene = () => {
    ...
    return (
        <div className="relative w-full h-full z-0">
            ...
            {!isDarkMode && (
                <div className="absolute inset-0 bg-white/55 z-10 pointer-events-none"></div> 
            )}
            ...
            <Canvas camera={{ position: cameraPosition, fov: 40 }}>
                ...
                <AnimatedBoxes 
                    boxColor={isDarkMode ? '#232323' : '#e0e0e0'} // Slightly darker color for light mode
                    boxOpacity={isDarkMode ? 1 : 0.65} // Increased opacity for light mode
                />
            </Canvas>
        </div>
    );
